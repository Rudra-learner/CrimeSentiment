...
BASE_URL = "https://timesofindia.indiatimes.com/city/bhubaneswar"
    ...
    try:
        ...
        if not published_date:
            try:
                ...
                for time_tag in soup.find_all("div", class_="byline"):
...
def collect_article_urls():
    article_urls = set()
    try:
        # TOI Bhubaneswar / Odisha news
        print(f"\nProcessing Page TOI Bhubaneswar")
        response = requests.get(BASE_URL, headers=HEADERS, timeout=20)
        soup = BeautifulSoup(response.text, "html.parser")

        for link in soup.find_all("a", href=True):
            href = link["href"]
            if "city/bhubaneswar" in href and href.endswith(".cms"):
